...
def Gen():
	...
	for t in range(32):
		...
		Image.fromarray(bg2).convert('L').point(lambda x: 0 if x<150 else 255).save('Im1/{}.jpg'.format(name))
...
def read_data_train():
    Gen()
    for fname in (glob.glob("Im1/*.jpg")):
...
def read_data_test():
    for fname in (glob.glob("Test1/*.jpg")[250:400]):
	im = Image.open(fname).convert('L').point(lambda x: 0 if x<150 else 255);
	im1 = np.array(im)/ 255.; code = fname.split("/")[-1][:-4]
        yield im1, code_to_vec(code)
...
def train(learn_rate, initial_weights=None):
    x, y, params = New_Model_Orig.build_model()
    ...
    with tf.Session(config=config) as sess:
        ...
        tf.train.write_graph(sess.graph_def, 'Model1/', 'graph.pb', False)
        writer = tf.summary.FileWriter('Model1', sess.graph)
        ...
        with open("Squeeze_Results1.csv",'wb') as f:
          ...
          while True:
            ...
            if batch_idx % 200 == 0:
               ...
               np.savez("Model1/PR_Squeeze_Weights_6.npz", *last_weights)
               minimal_graph_2 = convert_variables_to_constants(sess, sess.graph_def, ["output_node"])
               tf.train.write_graph(minimal_graph_2, '.', 'PR_Squeeze.pb', as_text=False)
            
f = np.load('Wts/pruned_weights_6.npz')
